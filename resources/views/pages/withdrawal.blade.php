            ...
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-3" id="kt_table_widget_4_table">
                    ...
                    <tbody class="fw-bold text-gray-600">
                        @forelse ($userWithdrawns as $key => $userWithdrawn)
                            <tr>
                                <td>{{ \Carbon\Carbon::parse($userWithdrawn->date)->format('Y-m-d') }}</td>
                                <td class="text-end">${{ $userWithdrawn->amount }}</td>
                                <td class="text-end">
                                    ${{ $userWithdrawn->fee }}
                                </td>
                                <td class="text-end">${{ $userWithdrawn->amount + $userWithdrawn->fee }}</td>
                            </tr>
                        @empty
                            <tr class="bg-light">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                    <form id="kt_modal_new_card_form" class="form"
                        action="{{ route('withdrawal.post', auth()->user()->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="d-flex flex-column my-7 fv-row">
                            ...
                            <input type="number" class="form-control form-control-solid" placeholder="Ex: 1000"
                                name="amount" required min="1" max="{{ auth()->user()->balance }}"
                                value="{{ old('amount') }}" />
                        </div>
